...
Ext.define('Ext.ux.grid.RowActions',{
    ...
    ,initRenderData:function() {
        ...
        if(me.groupActions && groupFeature) {
            ...
            function discardDuplicatedDivForGroupHeaderTpl(groupHeaderTpl) {
                var re = /^<div class=\"ux-grow-action-text\">/;
                if (typeof groupHeaderTpl == 'object') {
                    var headerString = groupHeaderTpl.html;
                    if (re.test(headerString)) {
                        var startString = headerString.indexOf("{");
                        var endString   = headerString.lastIndexOf("}");
                        groupHeaderTpl  = headerString.substr(startString, endString - startString +1);
                    }
                }
            ...
            groupFeature.groupHeaderTpl = 
                '<div class="ux-grow-action-text">'
                + discardDuplicatedDivForGroupHeaderTpl(groupFeature.groupHeaderTpl)
                + '</div>'
    ...
    ,onContainerEvent:function(view, e, options) {
        ...
        var records = grid.store.queryBy(function(r) {
            return r.get(groupField) == currentVal;
        });
